...
Now let's combine those last two tables to have a new table with the RFM value for all active customers (including those who never bought any item):
```{r}
...
Currently on this table we only have RFM and demographic data, we might also want sessions data, so let's add it:
```{r}
...
First, we decided to estimate all parameters excepted $m$ by values common to all customers. For $d$ or $T$, we have no reason to consider a different cost of capital or number of year out for a customer or another. For $AC$ we have no information concerning marketing operations that may have impacted it excepted the fall bonus, but we believe that it had little cost, little opportunity cost and had mostly an impact on $r$. On the opposite, we decided to estimate $m$ at a customer level based on past purchasing behaviour. For the purpose of this exercise we decided not to do the same technique for $r$ to keep a relatively simple model for CLV.
...
First we'll need to define when we consider that a customer has churned.
...
```{r}
all.customerID <- data_frame(CustomerID=customer$CustomerID)
tx.all$Month = format(tx.all$Date, "%m")
columns <- c('05', '06', '07', '08', '09', '10')
monthly.tx.all <- all.customerID %>%
...
# The answer
model <- lm(`10`~. - CustomerID, data=train)
...
```{r}
clv <- customer.active %>% mutate(CLV = CLV(CLV.const, model, CustomerID))
summary(clv)
```
...
Let's firt go thrgouh very basic descriptive analysis to get a few insights about the data: Player Profile, Age, Fallbonus or not and Income.
...
```{r}
ggplot(customer.RFM) + aes(x=Frequency,y=Monetary,col=CustomerType)+geom_point()+theme_classic()
ggplot(customer.RFM) + aes(x=Frequency,y=Monetary,col=Sex)+geom_point()+theme_classic()
ggplot(customer.RFM) + aes(x=Frequency,y=Monetary,col=Income)+geom_point()+theme_classic()
ggplot(customer.RFM) + aes(x=Frequency,y=Monetary,col=fallbonus)+geom_point()+theme_classic()
ggplot(customer.RFM) + aes(x=Frequency,y=Monetary,col=Age_category)+geom_point()+theme_classic()
```
