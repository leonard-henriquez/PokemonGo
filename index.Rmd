...
For a this approach we will have to consider CAC and CLV we previsouly calculated on Assignment 1 for each customer (note that we already have it in the customer.active dataset that we then merged with tx.all to get customer.LCG) 
Note that the CAC is always the same (this is the assumption we made in the first assignment because we don't have enough marketing financials information). 
...
### Calculation for CRM recommendation
