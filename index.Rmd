...
customer.LCG <- customer.LCG %>%
  mutate(Age=ifelse(between(Age, 6, 17), '6-17',
                    ifelse(between(Age, 18, 23), '18-23',
                          ifelse(between(Age, 24, 35), '24-35',
                                 ifelse(between(Age, 36, 50), '36-50', '>51')))))
customer.LCG$Age <- factor(customer.LCG$Age, levels=c('6-17', '18-23', '24-35', '36-50', '>51'))
customer.LCG
```
...
ggplot(lcg.6, aes(x=Age, y=quantity, fill=quantity))+
  theme_bw() +
  theme(panel.grid = element_blank())+
  geom_bar(stat='identity', alpha=0.6) +
  geom_text(aes(y=max(quantity)/2, label=value_ratio), size=3) +
